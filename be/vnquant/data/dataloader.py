    ...
    def download(self):
        ...
        if self.minimal:
            ...
            if str.lower(self.data_source) == 'vnd':
                ...
                data = stock_data[['High', 'Low',
                                   'Open', 'Close', 'adjust', 'Volume']]
                return data
        else:
            return stock_data
...
class DataLoaderVND(DataLoadProto):
    ...
    def download_one_new(self, symbol):
        start_date = utils.convert_text_dateformat(
            self.start, origin_type='%d/%m/%Y', new_type='%Y-%m-%d')
        end_date = utils.convert_text_dateformat(
            self.end, origin_type='%d/%m/%Y', new_type='%Y-%m-%d')
    ...
    def download_one(self, symbol):
        ...
        for i in range(1000):
            ...
            is_touch_end = utils.convert_date(
                self.start, '%d/%m/%Y') == utils.convert_date(date_end_batch, '%d/%m/%Y')
